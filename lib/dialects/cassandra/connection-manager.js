...
var AbstractConnectionManager = require('../abstract/connection-manager')
  ...
  , dataTypes = require('../../data-types').cassandra
  , parserMap = {};
...
ConnectionManager = function(dialect, sequelize) {
  ...
  this.sequelize.config.port = this.sequelize.config.port || 9042;
  try {
    if (sequelize.config.dialectModulePath) {
      this.lib = require(sequelize.config.dialectModulePath);
    } else {
      this.lib = require('cassandra-driver');
    }
  } catch (err) {
    throw new Error('Please install cassandra package manually');
  }
...
ConnectionManager.prototype.$refreshTypeParser = function (dataType) {
  dataType.types.cassandra.forEach(function (type) {
...
ConnectionManager.prototype.connect = function(config) {
  ...
  return new Promise(function (resolve, reject) {
    var connectionConfig = {
      ...
      policies: {
        loadBalancing: config.policies.loadBalancing,
        retry: config.policies.retry,
        reconnection: config.policies.reconnection,
        addressResolution: config.policies.addressResolution
      },
      queryOptions: {
        consistency: config.queryOptions.consistency,
        fetchSize: config.queryOptions.fetchSize
    ...
    var client = new this.lib.Client(connectionConfig);
